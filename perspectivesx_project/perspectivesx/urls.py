...
from .views import add_activity, index, student_submission, create_template, curate_item, LTIindex, LTInot_authorized, \
    UserSubmissionItemList, UserCuratedItemList,PerspectiveList, display_perspective_items, ActivityList,delete_submission_item,delete_curated_item
...
urlpatterns = [
    ...
    url (r'^display_perspective_items/(?P<activity>\d+)/(?P<perspective>\d+)/$', display_perspective_items, name = "display_perspective_items"),
    url(r'^get_user_curated_items/(?P<activity>\d+)/(?P<perspective>\d+)/$', UserCuratedItemList.as_view(), name="REST_user_cur_items"),
